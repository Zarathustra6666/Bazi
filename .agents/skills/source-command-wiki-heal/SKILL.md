---
name: "source-command-wiki-heal"
description: "Self-healing wiki pipeline: lint and fix the active wiki, detect concept/entity gaps, research and draft missing pages via WebSearch (MCP fallback), update index and log, then commit and push. Stops on merge conflicts and surfaces them to the user. Trigger on: /wiki-heal, \"self-heal wiki\", \"auto-maintain wiki\", \"heal the wiki\"."
---

# source-command-wiki-heal

Use this skill when the user asks to run the migrated source command `wiki-heal`.
...
Write `session-checkpoint.md` before starting (per AGENTS.md checkpoint protocol):
...
2. Draft a stub page using the appropriate template from AGENTS.md
