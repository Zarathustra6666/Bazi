---
name: "source-command-parallel-sync"
description: "Parallel multi-repo sync fleet. Discovers all git repos under C:\\Vault dynamically, spawns one subagent per repo, each linting and pushing, then reports a unified status dashboard. Never hardcodes the repo list. Trigger on: /parallel-sync, \"sync all repos\", \"parallel sync\", \"sync everything\"."
---

# source-command-parallel-sync

Use this skill when the user asks to run the migrated source command `parallel-sync`.
