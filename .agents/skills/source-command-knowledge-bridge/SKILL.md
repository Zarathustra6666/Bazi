---
name: "source-command-knowledge-bridge"
description: "Orchestrates a 4-tool knowledge-to-code pipeline connecting NotebookLM, Obsidian, VSCode, and Codex into a unified research-development workflow. Use this skill whenever the user wants to: bridge research notes with their codebase, convert NotebookLM exports or summaries into Obsidian notes, generate code scaffolds from Obsidian spec notes, sync VSCode project structure into an Obsidian knowledge graph, or run any workflow that spans research → notes → code. Trigger on phrases like \"sync my notes with my code\", \"create Obsidian notes from my research\", \"generate code from my spec notes\", \"document my project in Obsidian\", \"import NotebookLM to Obsidian\", \"link my vault to my project\", \"knowledge-to-code\", \"note-driven development\", or any mention of combining two or more of: NotebookLM, Obsidian, VSCode, Codex."
---

# source-command-knowledge-bridge

Use this skill when the user asks to run the migrated source command `knowledge-bridge`.
...
**VSCode** (code editor) → **Codex** (AI agent) into one fluid workflow.
...
- **Codex** = the agent doing the bridging (that's you, reading this skill)
...
## Design Notes
{any patterns, architecture decisions Codex can infer}
...
**When**: User has written a specification, feature plan, or design note in Obsidian and wants Codex to generate code from it.
...
# Last synced: {date}
# To regenerate: Codex "sync spec {note_name}" in project root
```
...
Generate `{project_path}/.Codex/sync-vault.sh`:
```bash
#!/bin/bash
# Knowledge Bridge — Vault Sync
# Run: bash .Codex/sync-vault.sh [--full | --notes | --code]
...
if [[ "$MODE" == "--full" || "$MODE" == "--code" ]]; then
  ...
  Codex "Run knowledge-bridge workflow 2 using project at $PROJECT and vault at $VAULT"
fi
  ...
  "tasks": [
    {
      ...
      "type": "shell",
      "command": "bash .Codex/sync-vault.sh",
      "group": "build",
      "presentation": {
        "reveal": "always",
        "panel": "new"
    ...
    },
    {
      ...
      "type": "shell",
      "command": "Codex 'Run knowledge-bridge workflow 1 — import notebooklm file at ${input:notebooklmFile}'",
      "group": "build",
      "inputs": [
        {
          "id": "notebooklmFile",
