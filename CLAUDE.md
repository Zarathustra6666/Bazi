...
This file is the operating manual for the LLM wiki agent. Every session starts by reading this file. Every operation follows the rules here. You (Claude Code) own the `wiki/` layer entirely — you create pages, update them, maintain cross-references, and keep everything consistent. The human curates sources, directs analysis, and asks questions.
