...
description: |
  ...
  โดยใช้ VSCode + Claude + NotebookLM + Obsidian ร่วมกัน
...
โดย Claude ทำหน้าที่เป็น Research Orchestrator ที่ประสานงานระหว่าง NotebookLM และ Obsidian
...
เมื่อผู้ใช้ให้หัวข้อมา ให้ Claude ทำสิ่งต่อไปนี้:
...
## ขั้นตอนที่ 3 — Synthesize (สังเคราะห์ด้วย Claude)

นำ Output จาก NotebookLM มาให้ Claude วิเคราะห์ต่อ
...
Claude สร้าง Note ในรูปแบบ Zettelkasten พร้อม Front matter
...
| Phase | เครื่องมือหลัก | Claude ทำอะไร | Output |
|-------|--------------|--------------|--------|
| Scope | Claude | กำหนด Questions + Keywords | Research Plan |
| Intake | NotebookLM | สร้างคำถาม 3 ระดับ | Question Set |
| Synthesize | Claude + NotebookLM | วิเคราะห์ + หา Gaps | Insight Report |
| Store | Obsidian | สร้าง Zettelkasten Notes | .md Files |
| Connect | Obsidian + Claude | สร้าง MOC + Backlinks | Knowledge Map |

---

## การติดตั้ง Skill นี้

### วิธีที่ 1 — Claude.ai Web
1. ไปที่ Settings → Customize → Skills
2. กด "Add Skill" → อัปโหลดไฟล์ `research-workflow-skill.md` นี้

### วิธีที่ 2 — Claude Code (VSCode)
```bash
mkdir -p ~/.claude/skills/research-workflow
cp research-workflow-skill.md ~/.claude/skills/research-workflow/skill.md
```

### วิธีที่ 3 — ใช้เป็น System Prompt
Copy เนื้อหาทั้งหมดไปใส่ System Prompt ของ Claude Project ที่ต้องการ
