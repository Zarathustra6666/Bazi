...
description: >
  ...
  raw PDFs never enter Claude's context (token optimization). Claude handles orient,
...
> NotebookLM is the extraction layer. Claude is the synthesis and writing layer.
> Raw PDFs and large documents never enter Claude's context — only NLM query responses do.
...
```
WebSearch          NotebookLM (MCP)           Claude
  │                    │                         │
...
> This is the token-saving step. Sources are indexed by NLM, not read into Claude's context.
...
Step 3b: Add each source URL
  ...
  For each URL from Phase 2 — NLM fetches and indexes; content never enters Claude's context
...
Follow the entity page schema in `CLAUDE.md`:
...
notebooklm-mcp-auth
```
Then restart the Claude Code session before retrying.
